...
export function registerBatchTools(server, godot) {
    ...
    server.tool("cross_scene_set_property", "Set a property on all nodes of a given type across ALL scene files in the project. Scans .tscn files, modifies matching nodes, and saves. Use for project-wide changes like updating all Label fonts or all Sprite2D modulate colors.", {
        type: z.string().describe("Node type to target (e.g. 'Label', 'Sprite2D')"),
