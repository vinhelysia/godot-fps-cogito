...
async function cmdInstall() {
    ...
    console.log("\nNext step: Run 'node setup.js configure' to set up your AI client.");
}
async function cmdCheckUpdate() {
    ...
    try {
        ...
        if (compareSemver(latest, current) > 0) {
            ...
            console.log("\nTo update: download the new version, replace server/src/, and run 'node setup.js install'");
        }
        else {
            console.log(`\nUp to date! (${current})`);
        }
...
async function cmdConfigure() {
    ...
    if (!existsSync(BUILD_INDEX)) {
        console.error("Server not built yet. Run 'node setup.js install' first.");
        process.exit(1);
    }
...
function cmdDoctor() {
    ...
    check("Server built", built, built ? BUILD_INDEX : "run 'node setup.js install'");
    // Version
    console.log(`\n  Version: ${getVersion()}`);
    // Overall
...
Examples:
  ...
  node setup.js configure
  node setup.js check-update`);
}
async function main() {
